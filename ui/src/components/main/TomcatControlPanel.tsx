...
export const TomcatControlPanel: React.FC<{ state: AppState, actions: AppActions }> = ({
    ...
    const getStartDisabledTitle = () => {
        ...
        if (state.tomcat.initializing) return "Tomcat이 초기화 중입니다.";
        if (state.build.isGradleRunning) return "빌드가 진행 중입니다.";
        if (isTransitioning) return "상태 변경 중입니다.";
        return undefined;
    };

    const getStopDisabledTitle = () => {
